...
public class AtorJogador {
	...
	public void receberSolicitacaoInicio(int posicao, String nomeJogador, String nomeAdversario) {
		interfaceGrafica.exibirMensagem("Partida iniciada", "O nome do seu adversário é " + nomeAdversario);
		tabuleiro.iniciarNovaPartida(posicao, nomeJogador, nomeAdversario);
		alterarConfiguracaoTabuleiro();
		
		if (posicao == 1)
			interfaceGrafica.exibirMensagem("Você começa", Constante.JOGADOR_COMECA_LANC);
		else
			interfaceGrafica.exibirMensagem("Seu adversário começa", Constante.ADVERSARIO_COMECA_LANC);
	}
	...
	public String obterCorPeaoEscolhida() {
		...
		if (interfaceGrafica.obterOpcao(opcoes,"Escolha a cor de seus peões", "Escolher cor") == 0)
